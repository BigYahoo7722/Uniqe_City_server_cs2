...
def fetch_workshop() -> list:
    ...
    نکته: Steam گاهی تو عنوان/توضیح نقشه‌ها کاراکترهای کنترلی نامعتبر (که تو
    XML مجاز نیستن) می‌فرسته و کل فید رو "not well-formed" می‌کنه. به‌جای اینکه
    مستقیم آدرس رو به feedparser بدیم (که خودش fetch+parse رو با هم انجام
    میده و کنترلی روی متن خام نداریم)، اول خودمون متن خام رو می‌گیریم، این
    کاراکترهای نامعتبر رو پاک می‌کنیم، بعد پاکشده رو به feedparser می‌دیم."""
    items = []
    try:
        log.info("در حال دریافت آیتم‌های ورک‌شاپ Steam ...")
        resp = requests.get(WORKSHOP_RSS_URL, headers=HTTP_HEADERS, timeout=20)
        resp.raise_for_status()
        # کاراکترهای کنترلی غیرمجاز تو XML 1.0 رو حذف می‌کنیم (تب/نیولاین/CR مجازن، بقیه نه)
